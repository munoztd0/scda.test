...
## Program Name:              tsfae03f.r
## R version:                 4.5.2
## junco Version:             0.1.3
## Short Description:         Program to create tsfae03f: Subjects With Treatment-
##                            emergent Infusion-related Reactions by System Organ
...
adae <- adae_jnj %>%
  ...
  filter(TRTEMFL == "Y" & toupper(AESCAT) == "INFUSION RELATED REACTION") %>%
  select(USUBJID, TRTEMFL, AEBODSYS, AEDECOD, AESCAT)
...
lyt <- lyt %>%
  analyze(
    ...
    extra_args = append(
      extra_args_rr2,
      list(val = "Y", label = "Subjects with >=1 infusion-related reactions")
    )
  ) %>%
  split_rows_by(
    "AEBODSYS",
...
if (length(adae$TRTEMFL) == 0) {
  result <- safe_prune_table(
    result,
    prune_func = remove_rows(removerowtext = "Subjects with >=1 infusion-related reactions")
  )
}
